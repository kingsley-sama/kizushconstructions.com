	...
	return (
		<div className='container mx-auto px-4 py-12 bg-white '>
			...
			<div className='flex lg:flex-row flex-col gap-2 items-center py-12'>
				<h2 className='text-4xl max-h-fit md:w-1/2 md:text-5xl font-bold text-center'>
					Our Portfolio
				</h2>
				<div className='max-w-l px-4  lg:pr-12'>
					...
					<button
						variant='outline'
						className='border-2 border-black hover:bg-white hover:text-white transition-colors'>
						View Mor Projects
					</button>
				</div>
			</div>
			...
			<div className='hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto'>
				{galleryItems.map((item, index) => (
					<div
						key={index}
						className='relative overflow-hidden rounded-lg group h-[200px]'>
						<img
							src={item.src || '/placeholder.svg'}
							alt={item.alt}
							className='w-full h-full object-cover transition-transform duration-300 group-hover:scale-105'
